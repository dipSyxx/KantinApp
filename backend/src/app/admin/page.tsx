...
export default async function AdminDashboard() {
  ...
  const [
    ...
    prisma.weekMenu.count({ where: { status: "PUBLISHED" } }),
    prisma.vote.groupBy({ by: ["value"], _count: { _all: true } }),
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        />
        <StatCard
          title="Totale stemmer"
          value={voteCount}
          icon={Vote}
          color="text-blue-600"
          bg="bg-blue-50"
          delta={votesThisWeek > 0 ? `+${votesThisWeek} denne uken` : undefined}
        />
        <StatCard
          title="Ukemenyer"
        ...
        <StatCard
          ...
          bg="bg-orange-50"
          delta={usersThisWeek > 0 ? `+${usersThisWeek} denne uken` : undefined}
        />
        <StatCard
          title="Stemmer i uka"
          value={votesThisWeek}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
          ...
          <div className="flex rounded-full overflow-hidden h-4 bg-gray-100">
            {voteDist.good > 0 && (
              <div
                ...
                style={{
                  width: `${(voteDist.good / voteTotal) * 100}%`,
                }}
              />
            )}
            {voteDist.ok > 0 && (
              <div
                ...
                style={{ width: `${(voteDist.ok / voteTotal) * 100}%` }}
              />
            )}
            {voteDist.bad > 0 && (
              <div
                className="bg-red-400 transition-all"
                style={{ width: `${(voteDist.bad / voteTotal) * 100}%` }}
              />
            )}
          </div>
          <div className="flex items-center gap-6 mt-3 text-sm">
            <span className="flex items-center gap-1.5">
              ...
              😀 {voteTotal > 0 ? Math.round((voteDist.good / voteTotal) * 100) : 0}%
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-full bg-amber-400" />
              😐 {voteTotal > 0 ? Math.round((voteDist.ok / voteTotal) * 100) : 0}%
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-full bg-red-400" />
              😞 {voteTotal > 0 ? Math.round((voteDist.bad / voteTotal) * 100) : 0}%
            </span>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
        <h2 className="text-xl font-bold mb-4">
          ...
          <span className="text-gray-500 font-normal text-base">
            {formatInTimeZone(now, "Europe/Oslo", "EEEE d. MMMM", {
              locale: nb,
            })}
          </span>
        </h2>
        {!todayMenu ? (
          <p className="text-gray-400 text-center py-6">
            Ingen meny for i dag
          </p>
        ) : !todayMenu.isOpen ? (
          <p className="text-gray-400 text-center py-6">
            Kantinen er stengt i dag
          </p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-3">
              {top3.map((dish, i) => (
                ...
                >
                  <span className="text-sm font-bold text-emerald-600 w-6">
                    #{i + 1}
                  </span>
                  ...
                  <span className="text-xs text-gray-400">
                    {dish.total > 0
                      ? Math.round(
                          ((dish.score + dish.total) / (2 * dish.total)) * 100
                        )
                      : 0}
                    % positiv
                  </span>
            ...
            <div className="space-y-3">
              {bottom3.map((dish, i) => (
                <div
                  ...
                  className="flex items-center gap-3 p-2 rounded-lg"
                >
                  <span className="text-sm font-bold text-red-400 w-6">
                    #{i + 1}
                  </span>
                  <span className="flex-1 font-medium text-sm truncate">
                    {dish.dishTitle}
      ...
      {/* 8. Recent votes with timestamps */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
      ...
      {delta && (
        <span className="block text-xs text-emerald-600 font-medium mt-1">
          {delta}
        </span>
      )}
    </div>
